...
local function compile_patterns()
  compiled_ignore = {}
  local all = {}
  for _, g in ipairs(DEFAULT_IGNORE_GLOBS) do table.insert(all, g) end
  for _, g in ipairs(config.extra_ignore_patterns or {}) do table.insert(all, g) end
...
function M.is_blocked_path(relpath)
  if not relpath or relpath == '' then return false, nil end
...
local REDACTIONS = {
  ...
  { kind = 'private_key_block', pattern = '%-%-%-%-%-BEGIN[^\n]-PRIVATE KEY%-%-%-%-%-.-%-%-%-%-%-END[^\n]-PRIVATE KEY%-%-%-%-%-' },
  { kind = 'secret_assignment', pattern = '[Aa][Pp][Ii][_%-]?[Kk][Ee][Yy]%s*[=:]%s*["\'][^"\']+["\']' },
  { kind = 'secret_assignment', pattern = '[Ss][Ee][Cc][Rr][Ee][Tt]%s*[=:]%s*["\'][^"\']+["\']' },
  { kind = 'secret_assignment', pattern = '[Tt][Oo][Kk][Ee][Nn]%s*[=:]%s*["\'][^"\']+["\']' },
  { kind = 'secret_assignment', pattern = '[Pp][Aa][Ss][Ss][Ww]?[Oo]?[Rr]?[Dd]?%s*[=:]%s*["\'][^"\']+["\']' },
